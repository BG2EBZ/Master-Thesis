...
class PolicySearchParamsTests(unittest.TestCase):
    def test_from_theta_accepts_five_dimensions_and_clips_explanation_scale(self):
        params = PolicySearchParams.from_theta(np.array([2.5, 3.5, 2.0, 0.7, 0.5], dtype=np.float64))
    ...
    def test_to_theta_outputs_five_dimensions(self):
        params = PolicySearchParams(explanation_time_scale=0.85)
        theta = params.to_theta()

        self.assertEqual(theta.shape, (5,))
        self.assertAlmostEqual(float(theta[4]), 0.85, places=7)

    def test_policy_summary_dicts_include_explanation_fields(self):
        legacy_theta = np.array([2.5, 3.5, 2.0, 0.7], dtype=np.float64)
        learned_theta = np.array([2.5, 3.5, 2.0, 0.7, 0.8], dtype=np.float64)
