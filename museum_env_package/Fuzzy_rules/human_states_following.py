"""
human_states_following_v5.py
Scikit-fuzzy implementation of human_states_following_v5.fis
...
# ─────────────────────────────────────────────
# Rules  (48 total, matching v5.fis exactly)
...
_RULE_TABLE = [
    ...
    # ── distracted rules ──────────────────────────────────────────────
    (1, 0, 3, 0,  0, 3, 0, 0),  # R09  short + hrd=far → dis=high
    (2, 0, 3, 0,  0, 2, 0, 0),  # R10  medium + hrd=far → dis=medium
    (3, 0, 3, 0,  0, 3, 0, 0),  # R11  long + hrd=far → dis=high
    (0, 0, 2, 3,  0, 3, 0, 0),  # R12  hrd=medium + crowded → dis=high
    (0, 0, 1, 3,  0, 2, 0, 0),  # R13  hrd=close + crowded → dis=medium
    ...
    # ── impatient rules ───────────────────────────────────────────────
    (3, 0, 3, 0,  0, 0, 3, 0),  # R17  long + hrd=far → imp=high
    (3, 0, 0, 3,  0, 0, 3, 0),  # R18  long + crowded → imp=high
    (3, 1, 0, 0,  0, 0, 3, 0),  # R19  long + hhd=close → imp=high
    (2, 0, 2, 3,  0, 0, 2, 0),  # R20  medium + hrd=medium + crowded → imp=medium
    (3, 0, 2, 1,  0, 0, 2, 0),  # R21  long + hrd=medium + low → imp=medium
    (1, 0, 0, 0,  0, 0, 1, 0),  # R22  short → imp=low
    (2, 0, 1, 1,  0, 0, 1, 0),  # R23  medium + hrd=close + low → imp=low
    (2, 0, 2, 2,  0, 0, 1, 0),  # R24  medium + hrd=medium + medium → imp=low
    # ── engaged=high rules ────────────────────────────────────────────
    (1, 0, 0, 1,  0, 0, 0, 3),  # R25  short + low → eng=high
    (1, 0, 0, 2,  0, 0, 0, 3),  # R26  short + medium → eng=high
    (2, 0, 1, 1,  0, 0, 0, 3),  # R27  medium + hrd=close + low → eng=high
    (2, 0, 1, 2,  0, 0, 0, 3),  # R28  medium + hrd=close + medium → eng=high
    (2, 0, 2, 1,  0, 0, 0, 3),  # R29  medium + hrd=medium + low → eng=high
    (3, 3, 1, 1,  0, 0, 0, 3),  # R30  long + hhd=far + hrd=close + low → eng=high
    (1, 3, 2, 1,  0, 0, 0, 3),  # R31  short + hhd=far + hrd=medium + low → eng=high
    # ── engaged=medium rules ──────────────────────────────────────────
    (2, 0, 2, 2,  0, 0, 0, 2),  # R32  medium + hrd=medium + medium → eng=medium
    (3, 3, 1, 2,  0, 0, 0, 2),  # R33  long + hhd=far + hrd=close + medium → eng=medium
    (3, 0, 2, 1,  0, 0, 0, 2),  # R34  long + hrd=medium + low → eng=medium
    (3, 0, 1, 1,  0, 0, 0, 2),  # R35  long + hrd=close + low → eng=medium
    (1, 3, 3, 1,  0, 0, 0, 2),  # R36  short + hhd=far + hrd=far + low → eng=medium
    (2, 3, 1, 2,  0, 0, 0, 2),  # R37  medium + hhd=far + hrd=close + medium → eng=medium
    (1, 3, 2, 2,  0, 0, 0, 2),  # R38  short + hhd=far + hrd=medium + medium → eng=medium
    (3, 0, 2, 2,  0, 0, 0, 2),  # R39  long + hrd=medium + medium → eng=medium
    (3, 2, 0, 2,  0, 0, 0, 2),  # R40  long + hhd=medium + medium → eng=medium
    # ── engaged=low rules (互斥/兜底压制) ─────────────────────────────
    (0, 0, 0, 3,  0, 0, 0, 1),  # R41  crowded → eng=low
    (3, 0, 3, 0,  0, 0, 0, 1),  # R42  long + hrd=far → eng=low
    (0, 1, 0, 3,  0, 0, 0, 1),  # R43  hhd=close + crowded → eng=low
    (3, 0, 0, 3,  0, 0, 0, 1),  # R44  long + crowded → eng=low
    (2, 0, 3, 0,  0, 0, 0, 1),  # R45  medium + hrd=far → eng=low
    (3, 1, 0, 2,  0, 0, 0, 1),  # R46  long + hhd=close + medium → eng=low
    # ── engaged default fallback ──────────────────────────────────────
    (0, 0, 0, 1,  0, 0, 0, 3),  # R47  density=low → eng=high  (兜底)
    (0, 0, 0, 2,  0, 0, 0, 3),  # R48  density=medium → eng=high (兜底)
]
